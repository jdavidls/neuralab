...
def causal_convolution(u, K, nofft=False):
    if nofft:
        ...
        assert K.shape[0] == u.shape[0], f'{K.shape}'
...
L = 200
A = make_HiPPO(32)
C = np.ones(32, dtype=complex)
Λ, P, B, _ = make_DPLR_HiPPO(32)
Ab, Bb, Cb = discretize(A, B, C, Δ)

Kb = K_conv(Ab, Bb, Cb, L)

K = kernel_DPLR(Λ, P, P, B, np.ones(32, dtype=complex), Δ, L)

#%%

rng = jax.random.PRNGKey(1)
u = jax.random.normal(rng, (1024,))

s4 = S4Layer(32, 1024, decode=True)
params = s4.init(rng, u)
y = s4.apply(params, u)
